  ...
  render() {
    ...
    return <div className="container">
        <div className="left-panel conversation-list">
          <ConnectionHeader connection={connection} onClick={() => {
          this.viewConversation(connection.id);
        }} />
          {connection.isWelcome ? <div className="below-header scrolling-panel">
              <h3 className="channel-list-heading">Conversations</h3>
              <ConversationList connectionId={connection.id} onSelectConversation={name => {
            this.viewConversation(name);
          }} selectedConversationId={conversation && conversation.name} />
              <JoinConversation onJoin={name => {
            dispatch(commandJoin(connection.id, [name]));
            this.viewConversation(name);
          }} />
            </div> : null}
        </div>
        {conversation ? <Conversation nickname={connection.nickname} conversation={conversation} onMessage={message => {
      ...
      }} disconnected={!connection.isConnected} /> : null}
      </div>;
  }
