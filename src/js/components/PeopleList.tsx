...
class PeopleList extends React.Component<Props, State> {
  ...
  render() {
    ...
    const peopleElements = people.filter(p => p.name.toLowerCase().includes(this.state.filter)).sort(byName).map((person, i) => <button className="nickname" key={i} onClick={() => {
    ...
    return <div className="people-list">
        ...
        <div className="header channel-header">
          <input type="text" placeholder={['search', people.length, people.length === 1 ? 'person' : 'people'].join(' ')} className="people-search-field" autoFocus onChange={this.handleChange.bind(this)} />
        </div>
