...
class Conversation extends React.PureComponent<Props, State> {
  ...
  render() {
    ...
    const peopleListEl = showPeopleList ? <PeopleList people={conversation.people} onCloseRequest={this.togglePeopleList.bind(this)} onPersonClick={this.props.onPersonClick} /> : null;
    return <div className={classNames('right-panel channel', {
      'with-details': showPeopleList
    })}>
        <div className="above-bottom-panel">
          <ConversationHeader onPeopleClick={this.togglePeopleList.bind(this)} dropdown={<ul className="pane conversation-dropdown">
                <li>
                  <label>
                    <input type="checkbox" checked={!filterStatusUpdates} onChange={() => {
                this.setState({
                ...
                <li>
                  <label>
                    <input type="checkbox" checked={showPeopleList} onChange={this.togglePeopleList.bind(this)} />
                    People List
                  </label>
                </li>
          ...
          <div className="below-header">
            {disconnected && <p className="disconnected-error">
                <AlertIcon width={18} height={18} stroke="#c51e21" fill="#c51e21" />
                <span>Connection lost.</span>
              </p> // but it's trying to reconnect.
          }
            <MessageList messages={filterStatusUpdates ? messages.filter(notStatusUpdate) : messages} />
          </div>
        </div>
        <div className="absolute-bottom-panel">
          <ComposeMessage nickname={nickname} onMessage={this.props.onMessage} people={conversation.people} />
        </div>
        {peopleListEl}
      </div>;
  }
...
const AlertIcon = props => <svg viewBox="0 0 100 100" {...props}>
    ...
    <path stroke="#FFF" strokeMiterlimit={10} d="M-178.5 22.5h30v30h-30zM-136.5 22.5h30v30h-30zM-93.5 22.5h30v30h-30zM-49.5 22.5h30v30h-30z" />
    <g>
