import { getConnectionById, getConversationsForConnection } from "../reducers/selectors";
...
export function commandJoin(connectionId: string, names: string[]): Dispatchable {
  return (dispatch, getState) => {
    const connection = getConnectionById(getState(), connectionId);
