class SelectList<A> {
  ...
  map<B>(fn: (arg0: A, arg1: boolean) => B): SelectList<B> {
    return new SelectList(this.before.map(a => fn(a, false)), fn(this.selected, true), this.after.map(a => fn(a, false)));
  }
  ...
  nextWrap(): SelectList<A> {
    ...
    if (this.after.length) {
      return new SelectList(this.before.concat(this.selected), this.after[0], this.after.slice(1)); // wrap to index 0
    } else if (this.before.length) {
      return new SelectList([], this.before[0], // everything but the first
      this.before.slice(1) // ...joined with everything else
      .concat(this.selected, this.after)); // nowhere to go
    } else {
      return this;
    }
  }
  ...
  prevWrap(): SelectList<A> {
    ...
    if (this.before.length) {
      return new SelectList(this.before.slice(0, this.before.length - 1), this.before[this.before.length - 1], [this.selected].concat(this.after)); // wrap to last index
    } else if (this.after.length) {
      return new SelectList(this.before.concat(this.selected, this.after.slice(0, this.after.length - 1)), this.after[this.after.length - 1], []); // nowhere to go
    } else {
      return this;
    }
  }
