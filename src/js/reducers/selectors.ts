...
export function getConnectionById(state: IrcState, connectionId: string): ConnectionT | null | undefined {
  return state.connections.list.find(({
    id
  }) => id === connectionId);
}
...
export function getSelectedConversation(state: IrcState, id: string): ConversationT | null | undefined {
  const connection = getConnectionById(state, id);
